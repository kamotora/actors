...
-import(common, [nop/1, send/2, say/2, sayEx/1, quoted/1, cookie/0, init/0, rand/1, rand/2, products/1]).
...
customer() -> Product = getRandomProduct(),
  ...
  receive
    {Price, "Refund"} -> sayEx(["Customer getting refund... :( "]);
    {Product} ->
      sayEx(["Customer getting ", quoted(Product)])
  after 5000 -> sayEx(["Customer timeout..."])
  end, timer:sleep(rand(500, 1500)), customer().

main() -> Customer_PID = spawn(fun() -> common:init(), customer() end),
